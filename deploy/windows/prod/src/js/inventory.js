...
$('#accNumberRow').on('click', '.btndelAuction', async function (e) {
    ...
    var result = await contractNFT.connect(signer).cancelAuctionByAuctionId(tokenId, {from: account});
...
$('#accNumberRow').on('click', '.btnSellToBidder', async function (e) {
    ...
    var account = await signer.getAddress(); 
    var result = await contractNFT.connect(signer).finalizeAuction(auctionId, {from: account});
    $progress.addClass("spa-loader")
    provider.waitForTransaction(result.hash).then((receipt) => {
        $progress.removeClass("spa-loader")
        console.log("Transaction Mined: " + receipt.transactionHash);
...
$('#btn-transfer').on('click', async function(e) {
    ...
    var tokenId = $('#tokenId').val();
      ...
      var result = await contractNFT.connect(signer).transferFrom(account, $('#addr').val(), tokenId, { from: account });
    $progress.addClass("spa-loader")
    provider.waitForTransaction(result.hash).then((receipt) => {
        $progress.removeClass("spa-loader")
        console.log("Transaction Mined: " + receipt.transactionHash);
...
$('#accNumberRow').on('click', '.btnDelete', async function (e) {
    ...
    var account = await signer.getAddress(); console.log(account + ',,,,,' + tokenId);
      var result=await contractNFT.connect(signer).burn(tokenId, {from: account});
    $progress.addClass("spa-loader")
    provider.waitForTransaction(result.hash).then((receipt) => {
        $progress.removeClass("spa-loader")
        console.log("Transaction Mined: " + receipt.transactionHash);
